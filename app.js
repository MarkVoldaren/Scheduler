const { parseCsv, parseCsvMatrix, isSchedulableRow, parseNumber, parseDate, buildJobs, normalizeGroup, calculateProductionDate, calculateOverallProgress, formatDate, startOfToday, stripTime, finalizeOperation, applyClaimCorrection, summarizeStatus, normalizeStatus, statusFromRank, createOperationKey, compareOperations, addDays, subtractWorkingDays, isWorkingDay, unique, clamp, isKnownDate } = globalThis.SchedulerCore;
...
async function loadServerAppState() {
  ...
  if (state.currentView === "projects") projectsUI.refresh();
}
